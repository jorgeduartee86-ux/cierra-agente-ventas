(function () {
  ...
  try {
    ...
    var expanded = raw.b ? { businessName: raw.b, agentName: raw.a, productName: raw.p, price: raw.r, description: raw.d, knowledge: raw.k, tone: raw.t, ctaLabel: raw.c, whatsapp: raw.w, phone: raw.h, instructions: raw.i, avatarData: raw.v } : raw;
  ...
  panel.innerHTML = '<header class="cierra-head"><div class="cierra-avatar">→</div><div class="cierra-id"><strong></strong><span><i></i></span></div><button class="cierra-close" aria-label="Cerrar chat">×</button></header><div class="cierra-messages" aria-live="polite"></div><div class="cierra-ctas"><a class="cierra-cta cierra-whatsapp" target="_blank" rel="noreferrer"></a><a class="cierra-cta cierra-call"></a></div><form class="cierra-form"><input aria-label="Mensaje" placeholder="Escribe tu pregunta…"><button class="cierra-send" aria-label="Enviar">→</button></form><div class="cierra-powered">Creado con <b>cierra.</b></div>';
  ...
  var whatsappCta = panel.querySelector(".cierra-whatsapp");
  var callCta = panel.querySelector(".cierra-call");
  var history = [];
  var busy = false;
  ...
  function showCta() {
    if (config.whatsapp) {
      whatsappCta.href = "https://wa.me/" + String(config.whatsapp).replace(/\D/g, "") + "?text=" + encodeURIComponent("Hola, me interesa " + config.productName);
      whatsappCta.textContent = "◉ " + config.ctaLabel;
      whatsappCta.classList.add("show");
    }
    if (config.phone) {
      callCta.href = "tel:" + String(config.phone).replace(/[^\d+]/g, "");
      callCta.textContent = "☎ Llamar ahora";
      callCta.classList.add("show");
    }
  }
