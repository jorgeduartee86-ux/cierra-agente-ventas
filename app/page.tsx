...
function ChatPreview({ config, compact = false }: { config: AgentConfig; compact?: boolean }) {
  ...
  async function sendMessage(text: string) {
    ...
    try {
      ...
      setMessages((current) => [...current, { role: "assistant", content: "Estoy teniendo una pausa breve. ¿Quieres continuar por WhatsApp?" }]);
    } finally {
      setLoading(false);
    }
  }
        ...
        <div className="studio-main">
          ...
          {tab === "test" && (
            <div className="studio-panel test-panel">
              <div className="panel-title"><div><small>Paso 2 de 3</small><h3>Hazle las preguntas difíciles</h3></div><span className="demo-status"><i /> Demo activa</span></div>
